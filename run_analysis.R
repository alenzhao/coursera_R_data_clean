    ...
    ## Request 5: From the data set in step 4, creats a second, independent tidy data set with the 
    ############## with the average of each varible for each activity and each subject.
    
    cateVec = paste0(bigTable[,1],",",bigTable[,2])
    tidyBy <- by(featureMeanStd,cateVec,FUN=colMeans)
    tidy_average <- do.call(rbind, tidyBy)
    subject_activity <-rownames(tidy_average)
    
    x= lapply(unique(cateVec), function(x) strsplit(x,","))
    cateVec <-do.call(rbind,x)
    
    colnames(cateVec) <- c("subject", "activity")
   
    tidy_average <-cbind(cateVec, tidy_average)
